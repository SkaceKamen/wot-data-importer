...
if ($update)
{
  ...
  while($file = readdir($dir))
  {
    if ($file!='.' && $file!='..')
    {
      ...
      Debug::log("Translation $ex(<i>$file</i>) loaded (keys: ".count($translation[$ex]).")", LEVEL_INFO);
    }
  }
}
...
function _T($key)
{
  ...
  if (substr($key,0,1) == '#')
  {
    ...
    if (isset($translation[$file][$_key]))
      return mysql_real_escape_string($translation[$file][$_key]);
    else
    {
...
foreach($equipment->children() as $node => $item) {
	...
	$data = array(
		...
		'name' => _T((string)$item->userString),
		'name_node' => $node,
		'description' => _T((string)$item->description),
		'icon' => $icon,
		'price' => $price,
  ...
  if ($nation!='.' && $nation!='..' && $nation != 'common')
  {
    ...
    foreach($tanks as $node=>$tank)
    {
      ...
      $data = array(
        ...
        'id' => (int)$tank->id,
        'name_node' => $node,
        'name' => _T((string)$tank->userString),
        'nation' => $nation,
        'class' => $class,
        'price' => (int)$price,
    ...
    Debug::log("Reading $nation\\radios ($file)");
    $list = simplexml_load_file($file);
    ...
    foreach($radios as $node=>$content)
    {
      ...
      $data = array(
		'wot_version_id' => $version_id,
        'name' => _T((string)$content->userString),
        ...
        'radio_distance' => (int)$content->distance
      );
      if ($mysql->insertOrUpdate($table,"name_node = '$node' AND wot_version_id = '$version_id'",$data))
      {
		Debug::log("<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
    ...
    foreach($items as $node=>$content)
    {
      ...
      if ($node != 'icons')
      {
        ...
        $data = array(
          ...
          'price' => (int)$price,
          'price_gold' => (int)$price_gold,
          'shell_type' => shelltype((string)$content->kind),
    ...
    foreach($items as $node=>$content)
    {
      ...
      foreach($shells as $s_node=>$s_content)
      {
		...
		if (!$shell_id) {
			Debug::log("\t<b>Error</b> Failed to find {$node2}", LEVEL_ERRORS);
		} else {
			$data = array(
    ...
    while($file = readdir($dir))
    {
      if ($file!='.' && $file!='..' && !in_array($file,$no_tanks))
      {
        ...
        foreach($list->hull->armor->children() as $node=>$content)
        {
          ...
          if ((bool)@$content->noDamage) { $armor[$node] .= '(true)'; }
        }
        ...
        foreach($items as $node=>$content)
        {
          ...
          foreach($content->armor->children() as $_node=>$_content)
          {
            ...
            if ((bool)@$_content->noDamage) { $armor[$_node] .= '(true)'; }
          }
			...
			if (!$gun_id) {
				...
				foreach($content2->armor->children() as $n=>$c)
				{
				  if ($n!='gun')
				  {
					...
					$armor[$n] = (int)$c;
					if ((bool)@$c->noDamage) { $armor[$n] .= '(true)'; }
				  } else {
					$gun_armor = (int)$c;
				  }
				}
        ...
        foreach($items as $node => $content)
        {
          ...
          if ($engine_id)
          {
			 ...
			 Debug::log("\t<b>Error</b> Failed to find {$node}", LEVEL_ERRORS);
		  }
