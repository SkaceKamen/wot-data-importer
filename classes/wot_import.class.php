...
final class wot_import
{
    ...
    function uploadMeta()
	{
        ...
        $this->deserializeWoTFilesForUse();

	}

    // BEGIN Version determining methods
	function getVersionNumberFromFolder($folderLocation)
	{
	...
	function getVersionExistsInDatabase($versionNumber, &$versionIdNumber)
	{
        // Todo: Migrate to stored procedure
        global $mysql;
    ...
    function deserializeWoTFilesForUse()
    {
    ...
    function deserializeWoTMetaFiles()
    {
    ...
    function importEquipment()
    {
        ...
        foreach($equipment->children() as $node => $item) {
            ...
            $equipment_id = $mysql->insertOrUpdate('wot_equipment',"name_node = '$node' AND wot_version_id = '$this->versionId'",$data);
            ...
            if ($nation!='.' && $nation!='..' && $nation != 'common')
            {
                ...
                $shellsData = function($shell,$node,$version_id)
                {
                    ...
                    return array(
                              ...
                              'id' => (int)$shell->id,
                              'name_node' => $node,
                              'name' => $this->TranslateToLocal((string)$shell->userString),
                              'price' => (int)$price,
                              'price_gold' => (int)$price_gold,
                              'shell_type' => $this->shelltype((string)$shell->kind),
                ...
                foreach($tanks as $node=>$tank)
                {
                    ...
                    if ($mysql->insertOrUpdate('wot_tanks',"name_node = '$node' AND wot_version_id = '$this->versionId'",$data))
                    {
                        Debug::log("<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
                        $new_tanks[] = $node;
                    } else {
                ...
                foreach($radios_ids as $node=>$content)
                {
                    ...
                    if ($mysql->insertOrUpdate($table, "name_node = '$node' AND wot_version_id = '$this->versionId'", $data))
                    {
                ...
                foreach($radios as $node=>$content)
                {
                    ...
                    $data = $radio_data($content,$this->versionId);
                ...
                foreach($items as $node=>$content)
                {
                    if ($node != 'icons')
                    {
                        ...
                        $data = $shellsData($content, $node, $this->versionId);
                ...
                foreach($items as $node=>$content)
                {
                    ...
                    $gun_id = $mysql->insertOrUpdate($table,"name_node = '$node' AND wot_version_id = '$this->versionId'",$data); 
                    if ($gun_id)
                    {
                        Debug::log("<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
                    } else {
                        $gun_id = $mysql->query("SELECT wot_items_guns_id FROM $table WHERE name_node = '$node' AND wot_version_id = '$this->versionId'");
                ...
                foreach($ids as $node=>$content)
                {
                    ...
                    if ($mysql->insertOrUpdate($table,"name_node = '$node' AND nation = '$nation' AND wot_version_id = '$this->versionId'",$data))
                    {
                        Debug::log("<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
                    } else {
                        Debug::log("<b>Update</b> {$node} " . print_r($data, true), LEVEL_INFO);
                ...
                while($file = readdir($dir))
                {
                    if ($file!='.' && $file!='..' && !in_array($file,$no_tanks))
                    {
                        ...
                        $tank_id = $mysql->insertOrUpdate('wot_tanks',"name_node = '$node' AND wot_version_id = '$this->versionId'",$data);
                        if ($tank_id)
                        {
                            Debug::log("<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
                        } else {
                        ...
                        foreach($items as $node=>$content)
                        {
                            ...
                            $turret_id = $mysql->insertOrUpdate($table,"name_node = '$node' AND wot_version_id = '$this->versionId'",$data);
                            if ($turret_id)
                            {
                                Debug::log("\t<b>Insert</b> {$node} " . print_r($data, true), LEVEL_INFO);
                            } else {
    ...
    function loopAndInsertOrUpdateItems()
    {
